...
final class BBBluetoothManagerTests: XCTestCase {
    ...
    func testUpdateConnectionCallCount() {
        ...
        let device = BluetoothObject(
            ...
            connected: .connected,
            type: BluetoothTypeObject(type: .headphones),
            battery: BluetoothBatteryObject(percent: 75),
        )
    ...
    func testDeviceListSimulation() {
        ...
        let devices = [
            BluetoothObject(
                ...
                connected: .connected,
                type: BluetoothTypeObject(type: .headphones),
                battery: BluetoothBatteryObject(percent: 80, left: 85, right: 75),
            ),
        ]
    ...
    func testBatteryLeftRightParsing() {
        ...
        let battery = BluetoothBatteryObject(percent: 80, left: 85, right: 75)

        // Then left and right values should be accessible
        XCTAssertEqual(battery.left, 85)
        XCTAssertEqual(battery.right, 75)
        XCTAssertEqual(battery.percent, 80)
    }

    func testBatteryWithoutLeftRight() {
        // Given a device without L/R battery (single battery device)
        let battery = BluetoothBatteryObject(percent: 60)

        // Then left and right should be nil
        XCTAssertNil(battery.left)
        XCTAssertNil(battery.right)
        XCTAssertEqual(battery.percent, 60)
    }
